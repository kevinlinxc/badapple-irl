...
# scale
first_video = cv2.VideoWriter('pre-tsp-video2.mp4', fourcc, 30, (width, height))
just_frame_number1 = cv2.VideoWriter("just_frame_number_pre-tsp2.mp4", fourcc, 30, (700, 100))
...
# actually use the same width and height as the first video so there's no weird cut
second_video = cv2.VideoWriter('tsp-video-finalwidthsameastsp.mp4', fourcc, 15, (width, height))
just_frame_number2 = cv2.VideoWriter("just_frame_number_tsp2.mp4", fourcc, 15, (700, 100))
...
for i in range(len(tsp_files)):
    ...
    # cv2.waitKey(0)
    print(f"tsp frames progress: {i}/3104, tsp order: {permutation_lookup[i]} file: {tsp_files[permutation_lookup[i]]}", flush=True)
